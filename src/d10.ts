...
function marchRay(
  map: Map, x: number, y: number, dx: number,
  dy: number): [number, number] | null
{
  x += dx;
  y += dy;
...
export function readMap(input: string): Map {
  ...
  let map = new Array2(rows[0].length, rows.length, Uint8Array);
  map.set(data);
...
export function findBestPos(
    map: Map, rays: [number, number][]
  ): [number, [number, number]]
{
  // visibility is used for debugging.
  let visibility = new Array2<Uint32Array>(map.width, map.height, Uint32Array);
  let best_pos: [number, number] = [-1, -1];
...
export function completeVaporization(
    map: Map, rays: [number, number][], x: number, y: number
  ): [number, number][]
{
  let removals: [number, number][] = [];
  let found = true;
