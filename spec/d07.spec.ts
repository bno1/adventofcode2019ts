import {runAmplifiers, runAmplifiers2} from "../src/d07";
import "jasmine";

describe("test exmple d07 star 1", function() {
  let cases: [number[], number[], number][] = [
    ...
    [[3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,1,33,31,
      31,1,32,31,31,4,31,99,0,0,0], [1,0,4,3,2], 65210],
  ];

  for (let [p, phases, thrust] of cases) {
    it(`Program ${p} with phases ${phases} produces thrust ${thrust}`, function() {
      expect(runAmplifiers(p, 0, phases)).toEqual(thrust);
    });
  }
});

describe("test exmple d07 star 2", function() {
  let cases: [number[], number[], number][] = [
    [[3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,27,4,27,1001,28,-1,28,1005,
      28,6,99,0,0,5], [9,8,7,6,5], 139629729],
    [[3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,-5,54,
      ...
      -1,56,1005,56,6,99,0,0,0,0,10], [9,7,8,5,6], 18216],
  ];

  for (let [p, phases, thrust] of cases) {
    it(`Program ${p} with phases ${phases} produces thrust ${thrust}`, function() {
      expect(runAmplifiers2(p, 0, phases)).toEqual(thrust);
    });
  }
});
